...
console.log("bun-plugin-react-native-testing-library secrets doctor");
console.log("======================================================");
console.log("Publish path: npm Trusted Publishing (OIDC) — no NPM_TOKEN after seed.");
...
if (npmInfo.error) {
  ...
  results.push({
    ...
    details: [
      ...
      "Or set a short-lived NPM_TOKEN for CI: bun run npm:seed -- --ci",
      "Then enable Trusted Publisher and delete NPM_TOKEN.",
    ],
  });
} else {
  results.push({
...
printCheck({
  id: "publish:oidc",
  status: "pass",
  message: "CI publish uses Trusted Publishing + id-token (no NPM_TOKEN after first seed)",
...
if (failedEntries.length > 0 || requiredChecklists.length > 0) {
  ...
  if (requiredChecklists.length > 0) {
    ...
    if (results.some((r) => r.id === "npm:registry" && r.status === "fail")) {
      console.log("  Seed the package first (one-time):");
